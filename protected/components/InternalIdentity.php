...
class InternalIdentity extends CUserIdentity
{
    ...
    public function authenticate()
    {
        ...
        if($record===null) {
			...
			$this->errorMessage = Yii::t('users', 'Sorry, But we can\'t find a member with those login information.');
		} else if( $record->password !== $record->hashPassword( $this->password, $record->email ) ) {
            $this->errorCode=self::ERROR_PASSWORD_INVALID;
			$this->errorMessage = Yii::t('users', 'Sorry, But the password did not match the one in our records.');
		} else {
            $this->_id = $record->id;
