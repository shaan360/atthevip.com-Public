...
class facebookIdentity extends CBaseUserIdentity
{
    ...
    public function authenticate( )
    {
        ...
        if($record===null)
		{
			...
			$this->errorMessage = Yii::t('users', 'Sorry, We could not find a member with that facebook account.');
		}
        else if( $record->email != $this->fbemail )
		{
            $this->errorCode=self::ERROR_USERNAME_INVALID;
			$this->errorMessage = Yii::t('users', 'Sorry, But the emails of the accounts did no match.');
		}
        else
        {
            $this->_id = $record->id;
