...
class ImageThumb {
	...
	public function __construct($cacheDir=null) {
		...
		if(!is_dir($this->cacheDir)) {
			...
			if(@mkdir($this->cacheDir)) {
				...
				$this->error = 'Error: Cache dir does not exits.';
				return false;
			}
		}
	}
	...
	public function run() {
		if (!isset($_GET['image']))
		{
			...
			$this->error = 'Error: no image was specified';
			return false;
		}
		
		$image = trim(urldecode($_GET['image']));
		...
		// Did we save the image to our local server earlier?
		$image = $this->getOriginalImage($image);
		...
		{
			header('HTTP/1.1 400 Bad Request');
			$this->error = 'Error: requested file is not an accepted type: ' . $image;
			return false;
		}
		...
		$quality	= (isset($_GET['quality'])) ? (int) $_GET['quality'] : $this->defaultQuality;
		...
		{
			$imageModified	= @filemtime($this->getOriginalImage($image));
			$thumbModified	= @filemtime($resized);
			
			//var_dump($imageModified, $thumbModified, date('d/m/Y H:i', $imageModified), date('d/m/Y H:i', $thumbModified));exit;
		...
		if (!is_readable($this->cacheDir))
		{
			...
			$this->error = 'Error: the cache directory is not readable';
			return false;
		}
		else if (!is_writable($this->cacheDir))
		{
			header('HTTP/1.1 500 Internal Server Error');
			$this->error = 'Error: the cache directory is not writable';
			return false;
		}
	...
	public function checkImageTooSmall($image, $minWidth=100, $minHeight=100) {
		$image = $this->getOriginalImage($image);
	...
	protected function getOriginalImage($image) {
		$originalImage	= sha1($image);
		
		// Get original saved file
		$file = $this->cacheDir . $originalImage;
	...
	protected function doConditionalGet($etag, $lastModified)
	{
